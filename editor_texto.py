...
class Editor(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Editor de Texto - Ermarly')
        #Confifuración tamaño mínimo de la venta
    ...
    def _crear_componentes(self):
        ...
        #Los botones los vamos a expandir de manera horizontal usando: (sticky='WE')
        ...
        # Frame - se coloca el Frame de manera vertical de arriba a abajo
    ...
    def _crear_menu(self):
        ...
        #Agregamos las subopciones del menú archivo
    ...
    def _abrir_archivo(self):
        #Abrir el archivo para edición (lectura-escritura)
    ...
    def _guardar_como(self):
        ...
        if not self.archivo:
            return
        #Si si tenemos un archivo valido, abrimos el archivo en modo escritura (write)
        with open(self.archivo, 'w') as self.archivo:
            #Leemos el contenido de la caja de texto
            texto = self.campo_texto.get(1.0, tk.END)
            #Escribimos el contenido al nuevo archivo
            self.archivo.write(texto)
            #Cambiamos el nombre del archivo para saber si el archivo se salvó
            self.title(f'Editor de Texto - {self.archivo.name}')
